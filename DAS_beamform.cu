__global__ void DAS_beamform(double* RF,double* RF_Cf,double* TimeDelayedRf,double* Cf_factor,double* channeldata,double* aperture_apo_w,double fs,
                                 double DepthOffset,double ct,double NoElem,double pitch,double* mask,double FineDelayInc,double IntFac,double lines,double samples,double R,double a)
                                 
{
	...
	if(idy<samples)
	{
		...
		for(int k = 0; k < (int)NoElem; k++) // Loop over aperture
		{
